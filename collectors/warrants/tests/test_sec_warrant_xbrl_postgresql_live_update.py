...
from collectors.warrants.src.warrants_postgresql import (
    write_sec_warrant_xbrl_facts,
)


# ============================================================
# QUANTLAB - SEC WARRANT XBRL RAW LIVE UPDATE TEST
# ============================================================
#
# Objectif :
#
...
def _count_test_rows(conn):
    with conn.cursor() as cur:
        cur.execute(
            "SELECT COUNT(*) FROM raw.sec_warrant_xbrl_fact WHERE cik = %s;",
            (TEST_CIK,),
        )
        return cur.fetchone()[0]


def test_sec_warrant_xbrl_capture_is_idempotent():
    conn = get_connection()
    ...
    try:
        ...
        first_pass = write_sec_warrant_xbrl_facts(
            conn,
            TEST_CIK,
            OBSERVATIONS,
            RETRIEVED_AT,
        ...
        second_pass = write_sec_warrant_xbrl_facts(
            conn,
            TEST_CIK,
            OBSERVATIONS,
            RETRIEVED_AT,
