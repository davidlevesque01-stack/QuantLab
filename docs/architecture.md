...
The Nasdaq Halt Collector currently contains the validated Python V0.7 processing pipeline.
...
The V0.7 pipeline preserves the source XML filename on each parsed event through:

```text
source_file
```

This provenance is persisted in:

```text
raw.nasdaq_trade_halt.source_file
```

The current V0.7 processing flow is:
...
unique_events
   |            |
   |            +----> Optional CSV / Validation Outputs
   |
   v
PostgreSQL RAW
        |
...
PostgreSQL CORE
        |
        v
Daily / Metrics / Analytics Processing
```

The production PostgreSQL path therefore no longer depends on processed CSV files.

Processed CSV datasets continue to be generated by V0.7 for:
...
- source-file provenance persistence;
...
This preserves the semantics of multi-day episodes observed in the Python processing baseline.

### Validated V0.7 Baseline

The current validated V0.7 dataset contains:
...
The V0.7 PostgreSQL integration has been validated with:
...
A subsequent execution against the same dataset validated idempotence:
...
Each structured RAW record contains the original XML source filename in:

```text
raw.nasdaq_trade_halt.source_file
```

The validated DEV dataset contains source filenames such as:

```text
tradehalts_2026-08-03.xml
tradehalts_2026-08-04.xml
...
tradehalts_2026-08-14.xml
```

The current validated distribution contains 744 RAW rows across 10 market-day XML files.

PostgreSQL stores the structured data and the source filename reference. It does not store the complete XML file as a database BLOB.

The original XML remains in the RAW filesystem storage and is retained for provenance and rebuildability.
...
This key is enforced by a UNIQUE constraint in PostgreSQL.

The Python V0.7 processing deduplication key is not identical to the PostgreSQL RAW natural key.

The current 744-event baseline contains no collision under either model.

This difference must be explicitly validated against the complete historical dataset before the model is considered final.
...
This relationship is valid for the current 744-event V0.7 baseline.
...
If an episode cannot be associated unambiguously with exactly one RAW event under the current model, persistence fails explicitly rather than arbitrarily selecting a RAW event.

The 1:1 model must therefore be revalidated against the complete five-year history before being considered permanent.
...
- equivalence with Python V0.7 metrics.

The current Python implementation derives market days from available dated XML files and weekday logic. This is sufficient for the current V0.7 validation baseline but is not considered an authoritative exchange calendar.
...
The current integration writes:
...
The complete RAW and CORE persistence operation executes through a PostgreSQL connection context so that database errors prevent a partially completed persistence operation from being treated as successful.

The integration is designed to be idempotent through PostgreSQL uniqueness constraints and conflict handling.
...
For example, distinct XML source files can be inspected through:
